...
void vimsyntaxgen_script_set_constant(const char* name, int value, bool isparameter) {
	...
	if ((underscore = strchr(name, '_'))) {
		...
		if (underscore < name)
			...
			len = underscore-name;
		if (len > 6 || len > sizeof(lastprefix) - 1 || atoi(name) != 0)
			len = 0;
...
void vimsyntaxgen_itemdb(void) {
	...
	for (i = 0; i < ARRAYLENGTH(itemdb->array); i++) {
		id = vimsyntaxgen_itemdb_search(i);
		if (!id || !id->name[0])
			continue;
...
void do_vimsyntaxgen(void) {
	...
	fprintf(local.fp,
		...
		"endif\n"
		"\n"
		"function! SyntaxCheckers_"SYNTAXLANGUAGE"_hercules_GetLocList()\n"
