...
void vimsyntaxgen_flush(int blanklines)
{
	...
	if (local.output.line[0] != '\0') {
		fprintf(local.fp, "%s%s%s\n", local.output.prefix[0] ? local.output.prefix : "", local.output.line,
		        local.output.terminator[0] ? local.output.terminator : "");
	}
...
void vimsyntaxgen_append(const char *str)
{
	...
	if (local.output.line[0] != '\0') {
		safestrncpy(local.output.line + strlen(local.output.line), local.output.separator,
		            sizeof(local.output.line) - strlen(local.output.line));
	}
	safestrncpy(local.output.line + strlen(local.output.line), str,
	            sizeof(local.output.line) - strlen(local.output.line));
}
...
void do_vimsyntaxgen(void)
{
	...
	fprintf(local.fp, "\"if exists(\"did_load_filetypes\")\n"
	                  "\"  finish\n"
	                  "\"endif\n"
	...
	fprintf(local.fp, "\" Only do this when not done yet for this buffer\n"
	                  ...
	                  "    let b:browsefilter = \"Athena Scripts (*.txt *.ath *." SYNTAXLANGUAGE
	                  " *." SYNTAXLANGUAGE ".txt)\\t*.txt;*." SYNTAXLANGUAGE ";*." SYNTAXLANGUAGE ".txt\\n\" .\n"
	                  "         \\ \"All Files (*.*)\\t*.*\\n\"\n"
	                  "endif\n"
	                  "\n"
	                  "let &cpo = s:cpo_save\n"
	...
	fprintf(local.fp, "\" Only load this indent file when no other was loaded.\n"
	...
	if ((local.fp = fopen(DIRECTORYNAME PATHSEP_STR
	                      "syntax_checkers" PATHSEP_STR SYNTAXLANGUAGE PATHSEP_STR SYNTAXLANGUAGE ".vim",
	                      "wt+"))
	    == NULL) {
	...
	fprintf(local.fp, "\" For version 5.x: Clear all syntax items\n"
	...
	fprintf(local.fp,
	        ...
	        "\" npc_unload\n"
	        "syn match	hDefaultLabel	display \"OnNPCUnload:\"me=e-1 contained\n"
	        "\" script_defaults\n"
	        ...
	        "syn match	hDefaultLabel	display \"OnUnTouch:\"me=e-1 contained\n"
	        "\" BUILDIN(calldynamicnpc)\n"
	        "syn match	hDefaultLabel	display \"OnDynamicNpcInit:\"me=e-1 contained\n"
	        "\n"
